...
{
    ONE_WIRE_LOW;
    Delay_us(480);       // 保持低电平480μs
    ONE_WIRE_HIGH;      // 释放总线
    Delay_us(70);        // 等待15-60μs后检测存在脉冲
    if (!ONE_WIRE_READ)
    {   // 检测到低电平（存在脉冲）
        Delay_us(410);   // 总复位时间至少480+70+410=960μs
        return 1;        // 复位成功，有设备响应
    }
    return 0;            // 无设备响应
}
...
{
	ONE_WIRE_LOW;
    Delay_us(bit ? 1 : 60);   // 写1时保持1μs，写0时保持60μs
    ONE_WIRE_HIGH;           // 释放总线
    Delay_us(bit ? 59 : 1);   // 补足60μs时隙
}
...
uint8_t OneWire_ReadBit()
{
    uint8_t bit = 0;
    ONE_WIRE_LOW;
    Delay_us(1);              // 拉低1μs启动读时隙
    ONE_WIRE_HIGH;           // 释放总线
    Delay_us(14);             // 等待14μs后采样
